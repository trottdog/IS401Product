...
- Start production backend with `npm run server:prod` (or `npm run start:prod`).
- Ensure `NODE_ENV=production`, `SESSION_SECRET`, and DB settings are configured.

## AWS Deployment (single Node app)

You can deploy the whole app (backend + Expo static output) as a single Node service on AWS (e.g. EC2, Elastic Beanstalk, or ECS/Fargate).

High‑level flow:

1. **Provision database**
   - Create a PostgreSQL instance (e.g. Amazon RDS).
   - Note the connection string and set it as `DATABASE_URL` (e.g. `postgres://USER:PASSWORD@HOST:PORT/DB_NAME`).

2. **Decide your public URL**
   - Decide the public host you’ll use (e.g. `byuconnect.yourdomain.com` behind an ALB / load balancer).
   - This value is used for `EXPO_PUBLIC_DOMAIN` when building the static assets.

3. **Build for production (run locally or in CI)**
   - From the project root:
     - `EXPO_PUBLIC_DOMAIN=byuconnect.yourdomain.com npm run expo:static:build`
     - `npm run server:build`
   - This produces:
     - `static-build/` – Expo static assets and manifests, served by the backend.
     - `server_dist/` – Bundled backend entry.

4. **Package artifacts for AWS**
   - Include at least:
     - `package.json`, `package-lock.json`
     - `server_dist/`
     - `static-build/`
     - `server/` (for templates) and `assets/` (if present)
   - You can:
     - Zip this and deploy to **Elastic Beanstalk (Node.js platform)**, or
     - Build a Docker image that runs `npm install && npm run start:prod` and deploy to **ECS/Fargate** or **a single EC2 instance**.

5. **Configure environment on AWS**
   - Set environment variables on the service:
     - `NODE_ENV=production`
     - `DATABASE_URL=postgres://USER:PASSWORD@HOST:PORT/DB_NAME`
     - `SESSION_SECRET=<strong-random-secret>`
     - (Optional) `CORS_ORIGIN=https://byuconnect.yourdomain.com`
   - Ensure the instance/task has outbound internet access to reach the database (or that your RDS security group allows the app’s security group).

6. **Run the app**
   - Start the server with:
     - `npm install`
     - `npm run start:prod`
   - The app listens on `PORT` (default `5000`) and `HOST` (default `127.0.0.1`); on AWS you typically:
     - Set `HOST=0.0.0.0`
     - Set `PORT` to the port your platform expects (e.g. `8080` on some PaaS stacks)

7. **Point your DNS**
   - Point `byuconnect.yourdomain.com` to your load balancer or instance.
   - Confirm that:
     - `https://byuconnect.yourdomain.com/` shows the landing page.
     - Expo Go clients can connect using the same domain (the build script bakes it into manifests).
